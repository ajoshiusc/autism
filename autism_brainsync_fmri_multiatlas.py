...
#    print(fmri_filenames)
 #   return
    fmri_filenames1=fmri_filenames['fmri_basc197']
    fmri_filenames2=fmri_filenames['fmri_msdl']

    a1 = np.array([pd.read_csv(subject_filename,
                              header=None).values
                  for subject_filename in fmri_filenames1])
    a2 = np.array([pd.read_csv(subject_filename,
                              header=None).values
                  for subject_filename in fmri_filenames2])
    a = a1
    for i in range(len(a)):
        a[i] = np.concatenate((a1[i], a2[i]), axis=1)
...
#        print(fmri_filenames['fmri_basc197'].index[0])
#        print(fmri_filenames.index[0])
#        if self.ref == 0:
#            self.ref = fmri_filenames['fmri_basc197'][fmri_filenames.index[0]]

        aa1 = pd.read_csv(fmri_filenames['fmri_basc197'][fmri_filenames.index[0]], header=None).values
        aa2 = pd.read_csv(fmri_filenames['fmri_msdl'][fmri_filenames.index[0]], header=None).values
        aa = np.concatenate((aa1, aa2), axis=1)
    ...
    def transform(self, X_df):
        fmri_filenames = X_df #[X_df['fmri_basc197'], X_df['fmri_msdl']]
#        fmri_filenames[0] = X_df['fmri_basc197']
#        fmri_filenames[1] = X_df['fmri_msdl']
...
class Classifier(BaseEstimator):
    def __init__(self):
        self.clf = make_pipeline(StandardScaler(), LogisticRegression()) # RandomForestClassifier(n_estimators=100, n_jobs=-1) 
